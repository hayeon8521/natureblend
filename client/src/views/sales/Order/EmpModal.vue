<template>
    <div>
            <!--모달 안에 영업직원 목록 출력-->
        <input type="text" v-model="inputListsearch" class="text-center styled-input" placeholder="직원명 입력하세요"/>
        <div class="grid-container">
            <ag-grid-vue
            
            :rowData="rowData"
            :columnDefs="columnEmplist"
            :theme="theme"
            @grid-ready="onReady"
...
export default{
    ...
    data(){
        return{
            ...
            theme : theme,
            columnEmplist : [
            {headerName : "직원번호", field:"emp_num",resizable: true, sortable: true},
            {headerName : "직원이름", field:"name",resizable: true, sortable: true},
            ],
            //검색어 검색 (그리드 안)
    ...
    methods: {
        async getEmpList(){
            ...
            this.rowData = result.data; //직원목록 저장
            console.log(this.empList);
        },

        selectemp(event){
            const emp = event.data;
