<template>
    <!--모달 안에 제품 목록 출력-->
    <!--모달 안에 제품 목록 출력-->
    <input type="text" v-model="inputListsearch" class="text-center styled-input" placeholder="반품코드 & 반품이유 입력하세요"/>
    <div class="grid-container">
        <ag-grid-vue
        
        :rowData="rowData"
        :columnDefs="columnReturnlist"
        :theme="theme"
        @grid-ready="onReady"
        :quickFilterText="inputListsearch"
        :noRowsOverlayComponent="noRowsOverlayComponent"
        @rowClicked="selectreturn"
        rowSelection="single"
    />
    </div>
...
export default{
    ...
    data(){
        return{
            ...
            theme : theme,
            columnReturnlist :[
            {headerName : "반품코드", field:"return_code",resizable: true, sortable: true},
            {headerName : "반품이유", field:"return_reason",resizable: true, sortable: true}
            ],
            //검색어 검색 (그리드 안)
    ...
    methods: {
        async getReturnList(){
            ...
            this.rowData = result.data; //반품목록 저장
            console.log(this.returnList);
        },

        selectreturn(event){
            const returns = event.data;
