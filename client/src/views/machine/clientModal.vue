<template>
<!--모달 안에 거래처 목록 출력-->
    <input type="text" v-model="inputListsearch" class="text-center styled-input" placeholder="거래처명 입력하세요"/>
...
export default{
    ...
    data(){
        return{
            ...
            columnClientlist : [
            {headerName : "거래처번호", field:"client_num",resizable: true, sortable: true},
    ...
    methods: {
        async getClientList(){
            let result = await axios.get(`${ajaxUrl}/machine/clientList`) //서버호출
